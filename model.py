...
def predict_demo(img):
    ...
    # 訓練済みモデル
    net_demo = Net()
    net_demo.load_state_dict(torch.load(
    ...
    # 結果を返す
    y_prob_demo = torch.nn.functional.softmax(torch.squeeze(y_demo))  # 確率で表す
    sorted_prob_demo, sorted_indices_demo = torch.sort(y_prob_demo, descending=True)  # 降順にソート
    return [(classes_ja[idx], classes_en[idx], prob_demo.item()) for idx, prob_demo in zip(sorted_indices_demo, sorted_prob_demo)]
...
def predict(img):
    ...
    net.load_state_dict(torch.load(
        "model3.pth", map_location=torch.device("cpu")
        ))
